      ...
      <nav className={`fixed w-full z-50 transition-all duration-500 border-b-[1.5px] border-white/80 ${isScrolled ? 'bg-brand-primary/95 py-3 top-0' : 'bg-transparent py-7 top-6'}`}>
        <div className="max-w-[1600px] mx-auto px-10 md:px-12 2xl:px-32">
          <div className="flex items-center">
            ...
            <div className="hidden xl:flex items-center gap-0.5 2xl:gap-1 grow justify-start h-[40px]">
              {navItems.map((item, index) => (
                item.isButton ? (
                  ...
                  <div key={item.label} className="group relative flex items-center h-full">
                    <Link 
                      to={item.path} 
                      className={`px-2 2xl:px-4 text-[13px] 2xl:text-[15px] font-serif font-bold uppercase tracking-[0.12em] transition-colors flex items-center gap-1 hover:text-brand-accent whitespace-nowrap relative py-2 ${
                        index === 1 
